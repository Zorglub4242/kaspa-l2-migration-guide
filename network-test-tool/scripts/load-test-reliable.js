...
async function main() {
  logger.success("✅ RELIABLE LOAD TEST - Guaranteed 100% Success");
  logger.gray("=".repeat(60));
  
  const transactionInput = await askQuestion(chalk.yellow("📊 How many transactions? (default 50): "));
  const TRANSACTION_COUNT = parseInt(transactionInput) || 50;
  
  const reliabilityInput = await askQuestion(chalk.yellow("🛡️ Reliability mode? (conservative/balanced/aggressive, default balanced): "));
  const RELIABILITY_MODE = reliabilityInput.toLowerCase() || 'balanced';
  
  if (TRANSACTION_COUNT < 1 || TRANSACTION_COUNT > 200) {
    logger.error("❌ Invalid transaction count! Must be between 1 and 200");
    process.exit(1);
  }
